module sui_nft_staking::nft_staking {
  ...
  struct PoolInfo<phantom CoinT, NftT: key + store> has key {
    id: UID,
  ...
  fun create_pool<RewardCoinT, NftT: key + store>(
    creator_address: address,
  ...
  public entry fun unstake_nft<RewardCoinT, NftT: key + store>(
    ...
    let NftStakeInfo { nft, stake_time, daily_reward } = nft_info_to_unstake;
  ...
  public entry fun claim_reward<RewardCoinT, NftT: key + store>(
    ...
    while (index < nft_count) {
      reward_sum = reward_sum + get_reward(vector::borrow(&user_info.nfts, index), user_info.last_reward_claim_time, cur_time);
      index = index + 1;
    };
    ...
    transfer::public_transfer(reward_to_claim, sender);
  }
  ...
  public entry fun withdraw_reward<RewardCoinT, NftT: key + store>(
    ...
    transfer::public_transfer(reward_to_withdraw, sender);
  }
